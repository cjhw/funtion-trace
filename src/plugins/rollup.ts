...
// 内嵌的追踪器代码（用于生产环境）
function getEmbeddedTracerCode(): string {
  return `// 内联的函数追踪器实现
class InlineTracer {
  constructor() {
    this.callStack = [];
...
export function functionTracerPlugin(options: PluginOptions = {}): Plugin {
  ...
  return {
    ...
    resolveId(id: string) {
      ...
      if (id === '@function-tracer/runtime') {
        return {
          id: path.resolve(__dirname, '../runtime/index.js'),
          external: false
        };
      }
      return null;
    ...
    load(id: string) {
      // 加载运行时模块
      if (id.endsWith('/runtime/index.js')) {
        // 在bundleless模式下，直接从源码读取
        const inlineTracerPath = path.join(__dirname, '..', 'runtime', 'inline-tracer.js');
        try {
          return readFileSync(inlineTracerPath, 'utf-8');
        } catch (error) {
          console.warn(`⚠️  [Function Tracer] 无法读取内联追踪器文件: ${inlineTracerPath}`, error);
          return getEmbeddedTracerCode();
        }
      }
      return null;
    },
    ...
    transform(code: string, id: string) {
      ...
      try {
        ...
        if (result.transformed) {
          trackedFiles.add(id);
          console.log(`🔧 [Function Tracer] 已插桩: ${path.relative(process.cwd(), id)}`);
    ...
    generateBundle() {
      if (trackedFiles.size > 0) {
        console.log(`✅ [Function Tracer] 构建完成，已追踪 ${trackedFiles.size} 个文件`);
  ...
  // 文件匹配辅助方法
  function shouldTransformFile(id: string, include: (string | RegExp)[], exclude: (string | RegExp)[]): boolean {
 ...
 * 插件工厂，根据环境自动选择配置
 */
export function createTracerPlugin(options: Partial<PluginOptions> = {}): Plugin {
  const isProduction = process.env['NODE_ENV'] === 'production';
